...
const training = ref<Omit<Training, 'id'>>({
  title: '',
  complexity: 'Средний',
  type: 'general',
  equipment: '',
...
const imageTraining = (event: Event) => {
      ...
      <div class="mb-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4" v-auto-animate>
          <div v-for="exercise in filteredExercises" :key="exercise.id" class="border p-4 rounded">
            ...
            <p class="text-sm text-gray-600 mb-2">{{ exercise.type }}</p>
            <p class="text-sm text-gray-600 mb-2">{{ exercise.equipment }}</p>
            ...
            <div class="mt-2">
              ...
              <input
                type="number"
                v-model.number="exerciseValues[exercise.id]"
                :class="{ 'border-red-500': isOverMaxValue(exercise) }"
                class="w-24 p-1 border rounded"
              />

              <div v-if="isOverMaxValue(exercise)" class="text-red-500 text-sm mt-1">
