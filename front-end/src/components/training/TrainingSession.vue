...
import isComplete from '../../components/technical/isComplete.vue'
...
function completeExercise() {
    if (!exercise.value || !currentExercise.value)
        return
            ...
            <template v-if="sessionStore.currentSession">
                <div v-if="currentExercise && exercise && !sessionStore.isResting" class="text-center">
                    ...
                    <div class="bg-white rounded-lg shadow-md p-6 mb-6">
                        <img :src="exercise.imageUrl" :alt="exercise.title" class="w-full max-w-md mx-auto mb-4 rounded">
                        ...
                        <div v-if="exercise.measurementType === 'time'" class="mb-4">
                            ...
                            <Button class="cursor-pointer mt-4 bg-blue-500 text-white px-6 py-2 rounded-lg"
                                @click="startTimer">
                                Запустить отсчёт
                            </Button>
                        </div>
                        ...
                        <div v-else class="mb-4">
                            <Input v-model.number="repetitions" type="number"
                                class="w-24 p-2 border rounded text-center text-xl" />
                            <h1 class="text-sm text-gray-500">
                                Цель: {{ currentExercise.goal }}
                            </h1>
                        </div>

                        <div class="flex max-sm:flex-col justify-center gap-4">
                            <Button class="cursor-pointer bg-blue-500 text-white px-6 py-2 rounded-lg"
                                @click="completeExercise">
                                Завершить
                            </Button>
                            <Button class="cursor-pointer bg-gray-500 text-white px-6 py-2 rounded-lg"
                                @click="skipExercise">
                                Пропустить
                            </Button>
                        </div>
                ...
                <div v-else-if="sessionStore.isResting" class="text-center">
                    ...
                    <div class="flex justify-center gap-4 mb-6">
                        <Button class="cursor-pointer bg-gray-500 text-white px-4 py-2 rounded"
                            @click="adjustRestTime(-10)">
                            -10s
                        </Button>
                        <Button class="cursor-pointer bg-gray-500 text-white px-4 py-2 rounded" @click="adjustRestTime(10)">
                            +10s
                        </Button>
                    </div>

                    <Button class="cursor-pointer bg-blue-500 text-white px-6 py-2 rounded-lg" @click="skipRest">
                        Пропустить отдых
                    </Button>
                </div>
                ...
                <div v-else class="text-center">
                    ...
                    <div class="bg-white rounded-lg shadow-md p-6 mb-6">
                        ...
                        <Button class="cursor-pointer inline-block bg-blue-500 text-white px-6 py-2 rounded-lg" as-child
