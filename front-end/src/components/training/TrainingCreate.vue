...
const exerciseValues = ref<Record<string, number>>({})
const measurementType = managementStore.measurementType
const imageFile = ref<File | null>(null)
const imagePreview = ref<string>('')
const complexityOptions = [
  { value: 'beginner', label: 'Новичок' },
  { value: 'intermediate', label: 'Профессионал' },
...
const filteredExercises = computed(() => {
  if (!selectedType.value) return exerciseStore.exercises
...
function isOverMaxValue(exercise: Exercise): boolean {
  const value = exerciseValues.value[exercise._id]
...
function addExercise(exercise: Exercise) {
  const value =
    exerciseValues.value[exercise._id] ||
  ...
  const existingIndex = training.value.exercises.findIndex(
    (exer) => exer.exerciseId === exercise._id,
  )
...
async function saveTraining() {
  try {
    ...
    await trainingStore.saveTrainingHandler(training.value, imageFile.value)
  } catch (error) {
...
<template>
  ...
  >
    <div class="flex flex-col justify-items-center max-w-4xl mx-auto p-4">
      <h1 class="text-2xl font-bold mb-6">Создание тренировки</h1>
      ...
      <div>
        ...
        <Input
          ...
          @change="imageTraining"
        />
        <img v-if="imagePreview" :src="imagePreview" alt="Preview" class="mt-2 max-w-xs rounded" />
      </div>

      <div>
        <label class="mt-4 block text-sm font-medium mb-2">Описание тренировки</label>
        <Textarea
          v-model="training.description"
          class="w-full p-2 mt-2 mb-4 border rounded focus:ring-2 focus:ring-blue-500"
        />
      </div>
      ...
      <div class="mb-6 mt-4">
        ...
        <select
          v-model="training.complexity"
          required
          class="w-full p-2 border rounded focus:ring-2 focus:ring-blue-500"
        >
          <option value="beginner">Выберите сложность</option>
      ...
      <div class="mb-6">
        ...
        <select
          ...
          class="w-full p-2 border rounded focus:ring-2 focus:ring-blue-500"
        >
          <option value="cardio">Выберите тип</option>
      ...
      <div class="mb-6">
        <h2 class="text-xl mb-4">Добавить упражнение</h2>
        ...
        <div class="flex flex-row space-x-4 items-center mb-4">
          ...
          >
            ...
            <option value="Торс">Торс</option>
            <option value="Бицепс">Бицепс</option>
          </select>
        </div>

        <div v-auto-animate class="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div v-for="exercise in filteredExercises" :key="exercise._id" class="border p-4 rounded">
            ...
            <div class="mt-2">
              ...
              <Input
                v-model.string="exerciseValues[exercise._id]"
                type="number"
                :class="{ 'border-red-500': isOverMaxValue(exercise) }"
            ...
            <Button
              class="cursor-pointer mt-4 bg-blue-400 hover:bg-blue-500 text-white px-4 py-1 rounded"
      ...
      <div class="mt-6">
        <h2 class="text-xl mb-4">Выбранные упражнения</h2>
        <div v-for="ex in training.exercises" :key="ex.exerciseId" class="mb-2 p-2 border rounded">
          <div class="flex flex-row justify-between justify-items-center items-center">
            ...
            <div class="flex flex-row items-center">
              ...
              >
              <Button
                class="cursor-pointer ml-4 text-red-500"
      ...
      <Button
        class="cursor-pointer mt-6 bg-blue-400 hover:bg-blue-500 text-white px-6 py-2 rounded"
        @click="saveTraining"
      >
